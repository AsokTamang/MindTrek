...
export async function GET() {
  ...
  const d = new Date(Date.now() - 20 * 24 * 60 * 60 * 1000); //here we are setting the varible called d which stores the time in milliseconds and i.e 7 days ago from the current data. but for example of showing my demo i have used just two days.
  ...
  try {
    ...
    const data = await collection
      .find({ userid: session?.user.id,createdAt:{$gte:d} })  //here we are finding all the mood datas from the past week.
